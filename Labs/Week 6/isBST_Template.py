...
def isBST(node, min_val=float('-inf'), max_val=float('inf')):
    ...
    if node:
        if node.left or node.right:
            if node.left and node.item > node.left.item:
                isBST(node.left, node.left.item, max_val)
            else:
                return False
            
            if node.right and node.item < node.right.item:
                isBST(node.right, min_val, node.right.item)
            else:
                return False
